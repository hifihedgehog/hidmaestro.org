...
3. **`[3/4]` `test\regression\swap_regression.ps1`.** The 41-scenario live-swap battery. Must report 41/41 PASS. A non-zero battery exit aborts with exit 1.
4. **`[4/4]` Cleanup + verdict.** Confirms no leftover devnodes and prints the safe-to-tag banner.

Total: ~30-40 minutes wall time. The regression battery dominates (devbox 41/41 in ~12 min, Atom Z8350 scale=2 in ~22 min).
...
| `profiles/` | 225 profile JSONs across 32 vendor folders |
