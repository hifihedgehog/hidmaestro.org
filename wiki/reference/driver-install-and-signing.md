...
Deployment has four cases. If the host controller that answers is HIDMaestro's own 0.9.7.7 and nothing has used it in this Windows session, the SDK moves it to 0.9.7.5: it stages the embedded usbip2_ude INF, SYS and CAT, verified against their pinned hashes, and the hmswd.exe helper forces that INF onto the controller with UpdateDriverForPlugAndPlayDevices under a time limit. The filter is not touched, and the 0.9.7.7 package is deleted only once the 0.9.7.5 image is the one the service loads. If any other usbip-win2 host controller answers, the SDK uses it as it is. If usbip-win2 is in the driver store and its host controller is not answering, the SDK restarts that device through CfgMgr and does not reinstall, because reinstalling would first remove the filter bound to every USB root hub. Only when no usbip-win2 exists does the SDK run the installer for the machine's architecture, silently, and wait for the host controller to appear. It never runs the vendor uninstaller.
