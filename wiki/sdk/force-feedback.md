...
Manually doing `.Descriptor(desc.Build()).InputReportSize(desc.InputReportByteSize + 1)` is correct and equivalent, but easy to get wrong: PadForge tracked this across several iterations of issue #16. The kernel sizes the input buffer wrong when the +1 is missing, HidClass preparsed data is misaligned, and `pid.dll` resolves Feature reports to Report ID 0 instead of the declared values.
...
The driver answers `HidD_GetFeature` for the canonical PID Report IDs (Pool 0x13, Block Load 0x12, State 0x14) directly from a per-controller shared state section the consumer fills via these methods. **No IPC round trip on the GetFeature path**: the driver reads from shared memory synchronously inside its IOCTL handler and completes the request.
...
Subsequent calls update pool state. Cheap: no driver IOCTL, just a shared-section seqlock write.
...
Returns a default-zero `HMPidBlockLoad` if `PublishPidPool` hasn't been called yet (FFB not enabled: the shared section doesn't exist).
...
Pre-1.1.40 the output channel was a single slot, latest-write-wins. That coalesced `pid.dll`'s tight three-packet bursts within 1-3 ms vs the SDK's 8 ms poll interval: middle packets dropped, magnitude never reached the consumer. Issue #16.

v1.1.40+ uses a **64-slot ring with monotonic seqlock per slot**. Writer (driver) increments a global Head counter, writes `slot[(Head-1) % N]`, readers track LastSeen and process slots from `LastSeen+1` to `Head`. The SDK's poll loop drains every slot the driver has written since the last poll, in monotonic SeqNo order: so all three bytes of a Set Effect &rarr; Set Constant Force &rarr; Effect Operation Start burst surface in the right order on the next poll iteration.

Ring depth is 64 slots × 256-byte payload. If the consumer's handler stalls for >512 ms while the driver is writing at burst rate, the oldest packets get overwritten. **Keep the handler cheap**: no synchronous I/O, no long locks. See [Output Passthrough](output-passthrough.md) for the wire format.
...
`pid.dll`'s follow-up `GetFeature(0x12 BlockLoad)` reads the freshly written values from the shared section: the driver's IOCTL handler reads the same memory the SDK consumer would read. The consumer then sees the SetFeature notification surface as `OutputReceived(HidFeature, 0x11)` ~8 ms later and reads the assigned EBI via `GetCurrentPidBlockLoad`.
...
- [HID Descriptor Builder](hid-descriptor-builder.md): `AddPidFfbBlock` reference and the four-feature pid.dll AV trap.
- [Output Passthrough](output-passthrough.md): the 64-slot ring buffer the FFB packets travel through.
- [Shared Memory Protocol](../reference/shared-memory-protocol.md): `HIDMAESTRO_SHARED_PID_STATE` wire format.
- [UMDF2 Driver Internals](../reference/umdf2-driver-internals.md): the `IOCTL_UMDF_HID_GET_FEATURE` path that reads from shared memory.
- [Testing and Verification](../reference/testing-and-verification.md): the S24-S26 regression scenarios end-to-end.

## References

- HID PID 1.0 specification (USB-IF; download from [usb.org/document-library](https://www.usb.org/document-library)): Report ID definitions (Pool §5.7, Block Load §5.5, State §5.8), Effect Operation §5.10, Block Free §5.11, Device Control §5.12.
- vJoy `Ffb_GetNextFreeEffect`: in [github.com/njz3/vJoy](https://github.com/njz3/vJoy) under `driver/sys/hid.c`. The kernel-side EBI auto-allocation pattern HIDMaestro mirrors in user mode.
- vJoy reference FFB descriptor: in the same repo under `driver/sys/hidReportDescFfb.h`. The four-feature variant `AddPidFfbBlock` deliberately omits.
- [HIDMaestro issue #16](https://github.com/hifihedgehog/HIDMaestro/issues/16): the pid.dll `PID_EffectOperation+0x52` AV.
- [`test/regression/swap_regression.ps1`](https://github.com/hifihedgehog/HIDMaestro/blob/master/test/regression/swap_regression.ps1): scenarios S24-S26 validate the FFB round-trip end-to-end.
