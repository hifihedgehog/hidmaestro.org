...
| **mshidumdf** | `mshidumdf.sys`: Microsoft's kernel-mode HID minidriver proxy. Acts as the function driver. The HID class stack (`HidClass.sys`) sees a real HID device; HIDMaestro.dll attaches as a UMDF2 lower filter under it. |
...
| **VHF** | Microsoft's Virtual HID Framework. Kernel-only. Mentioned for context: another approach HIDMaestro deliberately did not take. |
...
| Term | Meaning |
|------|---------|
| **HID** | Human Interface Device. The USB device class that gamepads, keyboards, mice, and joysticks use. The HID protocol is bus-agnostic: the same descriptor format works over USB, Bluetooth, BLE, and virtual buses. |
...
| **HIDAPI** | The libusb-maintained cross-platform HID library. Skips devices whose path contains `&IG_` (XInput-handled, by design). Detects Bluetooth via the device's CompatibleIDs: HIDMaestro spoofs `BTHLEDEVICE` for BT-mode profiles so HIDAPI reports `bus_type = BT`. |
...
| Term | Meaning |
|------|---------|
| **XUSB Companion** | A separate UMDF2 device (`HMXInput.dll`, INF: `hidmaestro_xusb.inf`) created alongside non-xinputhid Xbox profiles like Xbox 360 Wired. Lives at `SWD\HIDMAESTRO\<token>`. Registers the XUSB device interface and serves XInput IOCTLs. Needed because `mshidumdf` suppresses XUSB IOCTLs on HID devices. See [XUSB Companion](../reference/xusb-companion.md). |
| **xinputhid** | `xinputhid.sys`: Microsoft's inbox kernel filter for Xbox controllers using the GIP (Gaming Input Protocol) descriptor over HID. Binds as an upper filter on HID children matching `[GIP_Hid]` hardware IDs in `xinputhid.inf`. Provides XInput delivery and the 16-button HID descriptor synthesis natively. Used by Xbox Series / Xbox One / Xbox Elite v2 BT profiles. |
...
| **HMGamepadState** | The abstract input frame: every analog input in one `Dictionary<HMAxis, float> Axes` keyed by HID usage, values `[0.0, 1.0]` with 0.5 centered for sticks and 0.0 released for triggers, buttons bitmask, hat enum (or higher-resolution `HatDegrees` / `HatHundredths` / `HatRaw`). The SDK encodes into the active profile's HID descriptor. |
...
| **DriverStore** | `C:\Windows\System32\DriverStore\FileRepository\`. Where `pnputil` caches every installed driver package. Survives uninstall: budget for ~40 MB per HIDMaestro version churn on dev machines. |
...
| **Null-sentinel ContainerID** | `{00000000-0000-0000-FFFF-FFFFFFFFFFFF}`. Windows assigns this to ROOT-enumerated devnodes when the creator doesn't provide one. Triggers the bit-2 path in `xinput1_4!FUN_18000de2c` that skips XInput slot 0: the bug behind the historical "slot 1 is empty" symptom. The SWD migration assigns explicit non-sentinel ContainerIDs to bypass this branch. See [SwDevice and PnP](../reference/swdevice-and-pnp.md). |
...
| **Identity key** | The string a consumer passes to `CreateController(profile, identityKey)`. Every device path, the ContainerId and a USB/IP persona's serial derive from it, so the controller comes back at the same paths on every life. A missing key means the controller index. See [SwDevice and PnP](../reference/swdevice-and-pnp.md#stable-device-identity). |
| **Identity token** | The instance-name segment every devnode of one controller carries: `HM_0000` for the default key of index 0, `HM_` plus sixteen hex digits for a consumer key. |
| **ParentIdPrefix** | The `level&hash&n` value in a parent's instance key that PnP prepends to the instance id of every child that reports a non-unique id (HID children do). HIDMaestro writes the identity's value so the HID child keeps its path across lives. |
| **Sticky reuse-fast-path** | The empty devnode shell a `SwDeviceCreate` produced in releases before v1.1.31 when its `(enumerator + instanceId + ContainerId)` tuple matched a device that a `DIF_REMOVE` had left half-removed in the kernel. The handle-lifetime teardown removed the cause; since v1.8.0 every life reuses one fixed tuple on purpose. See [SwDevice and PnP](../reference/swdevice-and-pnp.md). |
...
| **EV certificate** | An "Extended Validation" code-signing certificate. Required by Windows for drivers signed for kernel mode. Costs $300+/year; project-fatal for hobbyists. **Not** required for HIDMaestro: UMDF2 + self-signed + TrustedPublisher trust path is sufficient. |
...
| **`bcdedit /set testsigning`** | The Windows boot flag that disables driver signature enforcement. **Not** required for HIDMaestro: the trusted self-signed cert is enough. Mentioned because every other "no EV cert" project on Windows requires it; HIDMaestro is the exception. |
...
- [Architecture Overview](../reference/architecture-overview.md): how all of the above pieces fit together.
- [Cross-API Coverage](../reference/cross-api-coverage.md): which terms apply at which downstream consumer.
- [Profile System](../profiles/profile-system.md): how `driverMode`, `connection`, and VID determine the runtime path.
