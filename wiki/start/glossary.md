...
| **Sticky reuse-fast-path** | The empty devnode shell a `SwDeviceCreate` produced in releases before v1.1.31 when its `(enumerator + instanceId + ContainerId)` tuple matched a device that a `DIF_REMOVE` had left half-removed in the kernel. The handle-lifetime teardown removed the cause, and since v1.8.0 every life reuses one fixed tuple on purpose. See [SwDevice and PnP](../reference/swdevice-and-pnp.md). |
