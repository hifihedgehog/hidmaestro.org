...
Composite personas use the bundled [usbip-win2 0.9.7.7 release](https://github.com/vadimgrn/usbip-win2/releases/tag/v.0.9.7.7). The build verifies each release installer's SHA-256, extracts its signed INF/SYS/CAT files, and verifies their hashes against `sdk/HIDMaestro.Core/UsbipPackage.json`. Both architecture packages remain inside the SDK DLL.
...
Run `scripts\verify_native_layout.cmd <usbip-win2-checkout>` with the reference checkout at tag `v.0.9.7.7` to compile those assertions for both architectures.
